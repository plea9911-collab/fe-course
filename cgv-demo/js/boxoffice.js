...
async function getPoster(movieNm, openDt) {
    ...
    let posters = await kmdb?.Data?.[0]?.Result?.[0]?.posters?.split('|');  //"?" = if와 같음. ?=response.json()이 null이 아니면 빠져나옴
    // console.log(posters);

    return posters?.[0];
}
    async function showList(list) {
         let slist = list.slice(0, 5).map(async (movie) => { //map같이 순회를 조건적으로 도는 것들은 break가 통하지 않기 때문에 break를 쓰지 않는다. 대신 slice를 씀 
