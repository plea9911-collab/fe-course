...
class Animal {
    //Field-변수, 상수 => static으로 정의, 클래스 변수 또는  상수로 부름
    //static으로 정의된 변수, 상수는 객체 생선 전 별도의 메모리에 저장됨 > 이미 저장되어 있기때문에 let, const❌ 필요없음 
    static MAX_SIZE = 10;  //대문자는 상수 //stack에 저장
    static className = 'Animal';
    name = undefined; // 1. 선언
    
    //Constructor- 객체 생성시 호출, new로 호출
    constructor(name, emoji) { //이것만 heap에 들어감
        this.name = name; // 2. 할당
        this.emoji = emoji;
    }
